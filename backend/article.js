...
articleRouter.get('/all', async (req, res) => {
    try {
        const queryAll = `
            SELECT a.article_id, a.title, a.description, a.slug, a.thumbnail_url, a.views, a.author, cont.profile_pic, c.name AS category_name, a.publish_at, a.likes
            FROM articles a
            JOIN categories c ON a.category_id = c.id
            JOIN contributor cont ON a.cont_id = cont.cont_id
            ORDER BY a.publish_at DESC
        `;
        const results = await db.query(queryAll);
        res.status(200).json({ articles: results[0] });
    } catch (error) {
        console.log(error);
...
// Category Articles
articleRouter.get('/category/:slug', async (req, res) => {
    const { slug } = req.params;

    try {
        const queryCheckCategory = `SELECT * FROM categories WHERE slug = ?`;
        const checkCategoryResults = await db.query(queryCheckCategory, [slug]);

        if (checkCategoryResults[0].length === 0) {
        ...
        const queryCategoryArticles = `
            ...
            ORDER BY a.publish_at DESC
        `;
        const results = await db.query(queryCategoryArticles, [slug]);
        const categoryQuery = `
        SELECT c.name, c.description, COUNT(a.article_id) AS article_count
        FROM categories c
        LEFT JOIN articles a ON c.id = a.category_id
        WHERE c.slug = ?
        GROUP BY c.name, c.description`;
        const categoryResults = await db.query(categoryQuery, [slug]);

        res.status(200).json({ articles: results[0], category: categoryResults[0] });
    } catch (error) {
        console.log(error);
